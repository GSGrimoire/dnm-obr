...
// BETA. Identical to background.js except that the modal opens the character
// creator itself rather than this repo's sheet.html, and the menu and modal ids
...
function setupContextMenu() {
  OBR.contextMenu.create({
    ...
    icons: [
      {
        ...
        label: "Attach D&M character (beta)",
        filter: {
          every: [
    ...
    onClick(context) {
      ...
      OBR.modal.open({
        id: `${ID}/sheet-modal${BETA}`,
        url: `${SHEET_URL}?item=${encodeURIComponent(item.id)}`,
        // Taller than the stable sheet: the creator's play view is a long
